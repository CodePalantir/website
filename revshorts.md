...
<section class="relative overflow-hidden bg-[linear-gradient(180deg,#FFFFFF_0%,#FBF9FF_55%,#FAF8F5_100%)]">
  ...
  <div class="relative z-[4] pt-32 md:pt-40 pb-10 md:pb-14 px-4 sm:px-6">
    <div class="max-w-wide mx-auto">
      ...
      <p class="text-muted mt-6 text-lg leading-relaxed max-w-2xl">Short reads on a single RevOps idea: one pattern, one failure mode, one fix. No filler, no consulting cycle. For the longer pieces, see the <a href="{{ site.baseurl }}/blog" class="font-bold text-accent hover:text-accent-strong transition-colors">blog</a>.</p>
    </div>
  </div>
</section>
...
<section class="pb-16 md:pb-24 px-4 sm:px-6 bg-paper">
  <div class="max-w-wide mx-auto">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
      {% for item in shorts %}
      <a href="{{ site.baseurl }}{{ item.url }}" class="apx-card group p-7">
    ...
    <div class="max-w-md apx-card border-dashed p-10 text-center">
      ...
      <p class="text-muted">First RevShorts are on the way. In the meantime, the <a href="{{ site.baseurl }}/blog" class="font-bold text-accent hover:text-accent-strong">blog</a> has the long-form pieces.</p>
    </div>
    {% endif %}
  </div>
</section>
